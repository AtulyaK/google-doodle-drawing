...
- **Current prototype:** The app now guides the player through one original two-stroke sigil, **First Binding**: a vessel ring followed by an apex mark. It represents **Water**, and a successful completion sends a short animated stream and a Water label across the stage before it clears. A normalized matcher allows ordinary variation in size, position, noise, and stroke direction while still rejecting incomplete or clearly wrong marks. Completing the sigil explains its meaning, plays a short optional cue, and automatically resets the drawing flow for another attempt. The earlier line, V, circle, and triangle recognizer remains the regression reference.
...
5. After the second stroke, the Water sigil completes automatically. A close attempt is accepted; an egregious miss gets a focused retry hint. On success, watch the water stream and its Water label cross the stage and fade away.
...
Select **Enable ambience** to loop a licensed medieval tavern track that is bundled with this repository; select **Mute ambience** at any time. Nothing is downloaded until you opt in, the recording is served from this app's own origin rather than hotlinked from a third party, and a short generated chime marks a completed sigil while ambience is on. If the browser cannot play the recording, the control reports itself as unavailable and the game continues in silence.
...
Open <http://localhost:5173/tests/line-speed.test.html> for current First Binding production-flow coverage through the stubbed camera path.
...
Open <http://localhost:5173/tests/ambient-audio.test.html> for ambience opt-in, mute, blocked-playback, and completion-cue checks. They use a fake media element, so no audio is downloaded or played.

Open <http://localhost:5173/tests/water-celebration.test.html> for the Water celebration's stream lifecycle, label, reduced-motion, cancel, and teardown checks.

## Credits and asset licences

- Ambience: [**"The Old Tower Inn"** by RandomMind](https://opengameart.org/content/medieval-the-old-tower-inn), from OpenGameArt, released under [CC0 1.0 Universal](https://creativecommons.org/publicdomain/zero/1.0/). CC0 waives attribution requirements; this credit is voluntary. The recording is bundled at `assets/audio/the-old-tower-inn.mp3` (about 2.4 MB) and is loaded only after you select **Enable ambience**.
- Everything else in this repository — the sigil, artwork, copy, animation, and the completion chime — is authored here.
- The MediaPipe hand-landmark runtime and model are loaded from their public CDNs at runtime under their own licences.
